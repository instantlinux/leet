...
def leetSolve(buildings):
    """skyline_problem algorithm

    Obvious way

    Args:
        buildings (list): list of 3-tuples defining position/heights
    Returns:
        list:  list of 2-tuple coordinates as defined
    """
    landscape = []
    for left, right, height in buildings:
        landscape += [0] * (right - len(landscape))
        for position in range(left, right):
            landscape[position] = max(landscape[position], height)
    results = []
    current = 0
    for position, level in enumerate(landscape):
        if (level != current):
